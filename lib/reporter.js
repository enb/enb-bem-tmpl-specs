...
Reporter.prototype.list = function (failures) {
    ...
    failures.forEach(function (test, i) {
        ...
        var fmt = color('error title', '  %s) %s:\n') +
            ...
            '     ' + color('diff added', 'expected') + ' ' + color('diff removed', 'actual\n') +
            '%s\n' + color('error stack', '\n%s\n');
        var err = test.err;
        ...
        var diffMessage = buildDiffMessage(diff);

        stack = stack.slice(index ? index + 1 : index)
            .replace(/^/gm, '  ');

        console.error(fmt, (i + 1), test.fullTitle(), msg, diffMessage, stack);
    });
};

function buildDiffMessage(diff, options) {
    ...
    diff.forEach(function (part) {
        var clr = part.added ? 'green' : part.removed ? 'red' : 'grey';
        ...
        if (part.value.length < showCharacters * 2) {
            ...
            if (indexOfPart < diff.length - 1) {
                output += '\n...\n' + color('diff gutter',
                    part.value.substr(part.value.length - showCharacters, showCharacters));
            }
        }
    });
